...
**Решение**: Заменен `npm ci` на `npm install --production --omit=dev` во всех Dockerfile с добавлением retry механизма и увеличенных таймаутов. Оптимизирована структура Dockerfile для ускорения сборки:
- Удалено копирование корневого `package.json` (который содержит лишние зависимости)
- Устанавливаются зависимости только из `package.json` конкретного сервиса
- Автоматически обновить зависимости согласно `package.json`
- Создать актуальный `package-lock.json` во время сборки
- Установить только production зависимости (без dev-зависимостей)
- Повторять попытки при сетевых ошибках (до 5 раз)
- Увеличенные таймауты для нестабильных сетевых соединений (до 5 минут)
- Улучшенное кэширование Docker слоев (меньше зависимостей = быстрее пересборка)
